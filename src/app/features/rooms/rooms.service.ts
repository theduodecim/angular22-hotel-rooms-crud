...
export class RoomsService {
  ...
  readonly roomsResource = httpResource<Room[]>(() => (this.hasStoredRooms() ? undefined : '/api/rooms'), {
    defaultValue: [],
  });
  ...
  updateRoom(updatedRoom: Room): void {
    const nextRooms = this.currentRooms().map((room) =>
      room.id === updatedRoom.id ? { ...updatedRoom, isSeed: room.isSeed ?? updatedRoom.isSeed } : room,
    );
